...
// Skills API calls
export const getProfessionalSkillById = async (skillId: string): Promise<any> => {
  const SKILLS_API_URL = import.meta.env.VITE_SKILLS_API_URL || 'https://api-skills.harx.ai/api';
  const response = await axios.get(`${SKILLS_API_URL}/professional-skills/${skillId}`);
  return response.data;
};

export const getTechnicalSkillById = async (skillId: string): Promise<any> => {
  const SKILLS_API_URL = import.meta.env.VITE_SKILLS_API_URL || 'https://api-skills.harx.ai/api';
  const response = await axios.get(`${SKILLS_API_URL}/technical-skills/${skillId}`);
  return response.data;
};

export const getSoftSkillById = async (skillId: string): Promise<any> => {
  const SKILLS_API_URL = import.meta.env.VITE_SKILLS_API_URL || 'https://api-skills.harx.ai/api';
  const response = await axios.get(`${SKILLS_API_URL}/soft-skills/${skillId}`);
  return response.data;
};

// Batch fetch skills
export const getSkillsDetails = async (skills: any[]): Promise<any[]> => {
  const SKILLS_API_URL = import.meta.env.VITE_SKILLS_API_URL || 'https://api-skills.harx.ai/api';
  ...
  const skillPromises = skills.map(async (skill) => {
    try {
      ...
      return {
        ...skill,
        skillDetails: skillDetails.data
      };
    } catch (error) {
      console.error(`Error fetching skill ${skill.skill || skill}:`, error);
      return {
        ...skill,
        skillDetails: { name: 'Unknown Skill', description: 'Skill not found' }
      };
    }
  });
